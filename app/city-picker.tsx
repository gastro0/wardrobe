...
const popular: City[] = [
  defaultCity,
  { name: "Санкт-Петербург", latitude: 59.93863, longitude: 30.31413 },
  { name: "Волгоград", latitude: 48.71939, longitude: 44.50183 },
  { name: "Казань", latitude: 55.78874, longitude: 49.12214 },
  { name: "Сочи", latitude: 43.59917, longitude: 39.72569 },
];

export default function CityPicker({ onClose, onSelect }: {
  onClose: () => void;
  ...
  function changeQuery(value: string) {
    setQuery(value);
    setCities(value.trim().length < 2 ? popular : []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api<{ cities: City[] }>(`/api/cities?q=${encodeURIComponent(query.trim())}`,
  ...
  async function choose(city: City | null) {
  ...
  return <Dialog open onOpenChange={open => { if (!open && !saving) onClose(); }}>
    <DialogContent className="city-dialog" showCloseButton={false}>
      ...
      <Combobox inline open items={cities} itemToStringLabel={(city: City) => city.name}
        ...
        <ComboboxInput showTrigger={false} className="city-input"
          placeholder="Начните вводить название" aria-label="Найти город"/>
        <div className="city-results">
          <ComboboxEmpty className="flex">{busy ? "Ищем города…" : error || "Город не найден. Попробуйте другое название."}</ComboboxEmpty>
          <ComboboxList>{(city: City) => <ComboboxItem
            key={`${city.latitude},${city.longitude}`} value={city}>
            <MapPin size={18}/><span><strong>{city.name}</strong>
              <small>{[city.admin1, city.country].filter(Boolean).join(", ")}</small></span>
