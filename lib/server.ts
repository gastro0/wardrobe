...
export function bucket():PhotoStorage {
  if(env.PHOTO_STORAGE==="supabase") {
    if(!env.SUPABASE_URL||!env.SUPABASE_SECRET_KEY||!env.SUPABASE_STORAGE_BUCKET)
      throw new ApiError(503,"Не удалось подключиться к фотографиям. Попробуйте ещё раз.");
    return createSupabaseStorage({url:env.SUPABASE_URL,secretKey:env.SUPABASE_SECRET_KEY,bucket:env.SUPABASE_STORAGE_BUCKET});
  }
  if(env.PHOTO_STORAGE&&env.PHOTO_STORAGE!=="r2")throw new ApiError(503,"Не удалось подключиться к фотографиям. Попробуйте ещё раз.");
  if(!env.BUCKET)throw new ApiError(503,"Не удалось подключиться к фотографиям. Попробуйте ещё раз.");
  return env.BUCKET;
}
...
export async function user(request:Request){
  ...
  const id=await verifyTelegramSession(readSessionCookie(request),env.TELEGRAM_BOT_TOKEN,new URL(request.url).origin);
  if(id===null)throw new ApiError(401,"Вход истёк. Закройте приложение и откройте его снова из Telegram.");
...
export function checkOrigin(request:Request){const origin=request.headers.get("origin");if(request.headers.get("sec-fetch-site")==="cross-site"||origin!==new URL(request.url).origin)throw new ApiError(403,"Не удалось проверить запрос. Обновите страницу.")}
export async function protect(request:Request){checkOrigin(request);return user(request)}
